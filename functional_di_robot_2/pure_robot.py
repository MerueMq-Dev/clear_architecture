"""
Низкоуровневые функции управления роботом.
Чистое ядро: не знает ни о клиентах, ни о конкретном способе передачи сообщений.
Зависимость `transfer` передаётся первым параметром в каждую функцию.
"""
import math
